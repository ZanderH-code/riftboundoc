...
describe("content pipeline contract", () => {
  it("runs normalize before schema validation", () => {
    ...
    const indexesIndex = script.indexOf('"tools/build_runtime_indexes.py"');
    ...
    expect(normalizeIndex).toBeLessThan(indexesIndex);
    expect(indexesIndex).toBeLessThan(validateIndex);
  });
});
